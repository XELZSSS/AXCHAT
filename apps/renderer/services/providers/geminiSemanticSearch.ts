import { ChatMessage, Citation, GeminiEmbeddingConfig, Role } from '../../types';
import { getActiveSessionId, getSession, getSessionSummaries } from '../sessionStore';
import { embedTextWithGemini, getResolvedGeminiEmbeddingConfig } from './geminiEmbeddings';
...
const loadSemanticCandidates = async (signal?: AbortSignal): Promise<SemanticSearchCandidate[]> => {
  ...
  return candidates
    .sort((left, right) => right.sessionUpdatedAt - left.sessionUpdatedAt || left.messageIndex - right.messageIndex)
...
export const retrieveGeminiSemanticContext = async ({
  ...
  const matches = scoredMatches
    ...
    .sort((left, right) => right.score - left.score || right.sessionUpdatedAt - left.sessionUpdatedAt)
