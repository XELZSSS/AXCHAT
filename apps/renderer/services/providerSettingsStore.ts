...
export const loadProviderSettings = (): Record<ProviderId, ProviderSettings> => {
  ...
  try {
    ...
    const parsed = JSON.parse(stored) as Partial<Record<string, Partial<ProviderSettings>>>;
    const currentProviderIds = new Set(listProviderIds());
    let didMigrate = Object.keys(parsed).some(
      (providerId) => !currentProviderIds.has(providerId as ProviderId)
    );
    ...
    if (didMigrate) {
      persistProviderSettings(defaults);
    }
    return defaults;
  } catch (error) {
